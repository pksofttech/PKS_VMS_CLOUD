...
let TOKEN_EXP = 24 * 60 * 60;

const debug = console.log;
const print_log = console.log;
...
async function fetchApi(path = "", method = "get", body = null, returnType = "text", header = true) {
    ...
    try {
        ...
        print_warn("fetchApi not response or error: " + err);
        return null;
    } finally {
        //swal.close();
    }
    ...
    switch (response.status) {
        case 401:
            ...
            const alert = await Swal.fire({
                icon: "error",
                title: "Oops... session expires or user not allow",
                text: "กรุณาลงชื่อเข้าใช้งานระบบ",
            });
            location.reload();
